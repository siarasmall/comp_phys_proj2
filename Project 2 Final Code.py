...
class WaveFunctionSimulation:
  ...
  def create_evolution_matrix_1(self, periodic_boundary, potential_values) -> np.ndarray:
    '''
    Creates a matrix that evolves the wavefunction over time using the first method.
    ...
    # Solve for psi_{n+1}
    total = I + 0.5*self.dt/(1j*self.dx**2) * M - self.dt/(1j) * V

    return np.linalg.inv(total)

  def create_evolution_matrix_2(self, periodic_boundary, potential_values) -> np.ndarray:
    '''
    Creates a matrix that evolves the wavefunction over time using the second method.
  ...
  def simulate(self, method, periodic_boundary, potential_values, wave_center, wave_width, wave_momentum) -> np.ndarray:
    '''
    Simulates a wave function in the given potential values using the second algorithm.

    Params:
      - method (int): The algorithm to use (1 for first method, 2 for second method).
      - periodic_boundary (bool): Whether to use periodic boundary conditions.
      - potential_values (list): The list of potential values at each x point.
      - wave_center (int): The x position of the center of the initial wavepacket.
      - wave_width (int): The width of the initial wavepacket.
    ...
    self.evoluton_matrix = None
    if method == 1:
      self.evoluton_matrix = self.create_evolution_matrix_1(periodic_boundary, potential_values)
    elif method == 2:
      self.evoluton_matrix = self.create_evolution_matrix_2(periodic_boundary, potential_values)
    else:
...
def animate_simulation(x, sim_data, potential_values, frames):
  # Print out the total probability of the initial and final wavefunctions
  # Used to highlight difference between both methods
  init_prob = round(np.trapz(np.abs(sim_data[0])**2, x),1)
  final_prob = round(np.trapz(np.abs(sim_data[-1])**2, x),1)
  print(" INITIAL TOTAL PROBABILITY | FINAL TOTAL PROBABILITY ")
  print("|--------------------------|------------------------|")
  print(f"|           {init_prob}            |           {final_prob}          |")
...
**Use the simulate function from the WavefunctionSimulation class to run the simulation:**\
    *Specify the method number to use (1 or 2), the boundary conditions, potential, and initial wavepacket values* \

    simulated_wavefunctions = simulation.simulate([Method #], [Periodic Boundary Conditions], [potential_aryay], [wavepacket center], [wavepacket width], [wavepacket momentum])
...
free_particle_sim = Simulation.simulate(1, True, potential_values, -5, 1, 2)
...
free_particle_sim = Simulation.simulate(2, True, potential_values, -5, 1, 2)
...
potential_barrier = Barrier(0, 5, 0, 2, SIM_POINTS, X_VALUES)
potential_values = potential_barrier.getMagnitudes()

barrier_sim = Simulation.simulate(2, True, potential_values, -12, 1, 2)
...
finite_well = FiniteSquareWell(-5, 5, 0, 1, SIM_POINTS, X_VALUES)
potential_values = finite_well.getMagnitudes()

finite_well_sim = Simulation.simulate(2, True, potential_values, 0, 1, 0)

ani1 = animate_simulation(X_VALUES, finite_well_sim, potential_values, SIM_STEPS)
...
infinite_well_sim = Simulation.simulate(2, True, potential_values, 0, 1, 0)

ani1 = animate_simulation(X_VALUES, infinite_well_sim, potential_values, SIM_STEPS)

ani1

"""### Harmonic Oscillator
Wavefunction starting in the center
"""

quantum_SHO_middle = SimpleHarmonicOscillator(X_VALUES, 0, 0.1)
potential_values = quantum_SHO_middle.getMagnitudes()

quantum_SHO_middle_sim = Simulation.simulate(2, True, potential_values, 0, 1, 0)

ani1 = animate_simulation(X_VALUES, quantum_SHO_middle_sim, potential_values, SIM_STEPS)

ani1

"""### Harmonic Oscillator
Wavefunction starting off-center
"""

quantum_SHO_offcenter = SimpleHarmonicOscillator(X_VALUES, 0, 0.5)
potential_values = quantum_SHO_offcenter.getMagnitudes()

quantum_SHO_offcenter = Simulation.simulate(2, True, potential_values, -5, 1, 0)

ani1 = animate_simulation(X_VALUES, quantum_SHO_offcenter, potential_values, SIM_STEPS)
...
kronig_penney_sim = Simulation.simulate(2, True, potential_values, -10, 1, 2)

ani1 = animate_simulation(X_VALUES, kronig_penney_sim, potential_values, SIM_STEPS)
